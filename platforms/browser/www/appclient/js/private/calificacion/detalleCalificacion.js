...
function cargarDetalle() {
    ...
    httpConnect("http://10.75.199.57:88/calificacion/" + id, null, "GET", function (r) {
        if (r.status !== 200) {
            alert(r.message);
            window.location.replace("?p=calificacion/listarCalificacion");
        }

        cargarValoracion();
        $("#calComentario").val(r.data.calComentario);
        cargarAlojamiento();
        cargarUsuario();
...
function cargarAlojamiento() {
    ...
    httpConnect("http://10.75.199.57:88/alojamiento", null, "GET", function (r) {
        for (var i = 0; i < r.data.length; i++) {
            var o = r.data[i];
...
$(function () {
    ...
    $("#btnUpdateCalificacion").click(function () {
        var correcto = true;
        ...
        var id = $("#id").val();
        httpConnect("http://10.75.199.57:88/calificacion/" + id, jentidad, "PUT", function (r) {
